...
echo "====== 开始部署 WVP-PRO 全流程环境 (适配新版目录结构) ======"
...
echo "=== 编译前端页面 ==="
if [ -d "web_src" ]; then
    # 新版结构：web_src为源码目录，编译输出到web
    echo "检测到新版前端结构 (web_src/)"
    cd web_src
    npm install --force --no-audit --fund=false --progress=false
    npm run build
    cd ..
elif [ -d "wvp-pro-web" ]; then
    # 旧版结构
    echo "检测到旧版前端结构 (wvp-pro-web/)"
    cd wvp-pro-web
    npm install --force --no-audit --fund=false --progress=false
    npm run build
    cd ..
    ...
    echo "错误：找不到前端目录 (尝试过: web_src/, wvp-pro-web/, web/)"
    exit 1
fi

# 确保web目录存在
if [ ! -d "web" ]; then
    echo "错误：前端编译失败，web目录未生成"
    exit 1
fi
...
# 检查前端资源
echo "前端资源检查："
ls -lh /opt/wvp-GB28181-pro/web/static/js/ 2>/dev/null || echo "警告：前端资源未找到"
